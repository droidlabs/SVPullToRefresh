...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        SVPullToRefreshView *view = [[SVPullToRefreshView alloc] initWithFrame:CGRectMake(0, self.pullToRefreshView.yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight)];
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, self.pullToRefreshView.yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        if (!self.yOrigin) {
            self.yOrigin = -SVPullToRefreshViewHeight;
        }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, self.yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        if (self.customOffsetThreshold) {
            scrollOffsetThreshold = self.customOffsetThreshold;
        }
